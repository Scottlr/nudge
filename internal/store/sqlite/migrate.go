...
func migrationCatalog() ([]migration, error) {
	...
	for index, name := range entries {
		...
		if strings.Contains(name, "owned_storage") {
			...
			owner = "git_proposals"
		} else if strings.Contains(name, "repair_framework") {
			owner = "build_nudge_platform"
		} else if strings.Contains(name, "workspace_retirement") {
			owner = "git_proposals"
		} else if strings.Contains(name, "session_lease") {
			owner = "persist_nudge_state"
